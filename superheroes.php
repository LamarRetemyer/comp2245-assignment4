...
$search = isset($_GET['query']) ? strtolower($_GET['query']) : '';
$searchWords = preg_split('/\s+|-/', $search, -1, PREG_SPLIT_NO_EMPTY);
$foundSuperhero = null;

foreach ($superheroes as $superhero) {
    $superheroName = strtolower($superhero['name']);
    $superheroAlias = strtolower($superhero['alias']);

    if (array_reduce($searchWords, function ($carry, $word) use ($superheroName, $superheroAlias) {
        return $carry || strpos($superheroName, $word) !== false || strpos($superheroAlias, $word) !== false;
    }, false)) {
...
if ($foundSuperhero) {
    ...
    $imageName = str_replace(' ', '', $foundSuperhero['alias']);
    $imagePath = "Avengers images/{$imageName}.jpg";
    echo "<div class='hero-image'>";
    echo "<img src='{$imagePath}' alt='{$foundSuperhero['alias']}'>";
    echo "</div>";
